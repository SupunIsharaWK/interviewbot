...
def training_decoding_attention(rnn_size, enc_output, enc_state, input_length,
    ...
    attn_mech_training = tf.contrib.seq2seq.BahdanauAttention(rnn_size, enc_output, input_length, normalize=False, name='BahdanauAttention')
    dec_cell_training = tf.contrib.seq2seq.AttentionWrapper(cell=dec_cell, attention_mechanism=attn_mech_training, attention_layer_size=rnn_size)
...
def training_decoding_layer(dec_embed_input, target_length, dec_cell, initial_state,
    ...
    training_helper = tf.contrib.seq2seq.TrainingHelper(inputs=dec_embed_input, sequence_length=target_length, time_major=False)
    training_decoder = tf.contrib.seq2seq.BasicDecoder(dec_cell,
                                                       training_helper,
                                                       initial_state,
                                                       output_layer)
    training_logits = tf.contrib.seq2seq.dynamic_decode(training_decoder, output_time_major=False, impute_finished=True, maximum_iterations=max_target_length)
