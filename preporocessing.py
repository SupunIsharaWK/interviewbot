...
def save_pickle_file(data, filename):
    ...
    pickle.dump(data, save_documents, protocol=2)
    save_documents.close()


def load_pickle_file(data_filepath):
...
def preprocess_data(data, remove_stopwords=True, replace_entities=False):
    ...
    for index in range(len(data)):
        ...
        if index % 1000 == 0 and index > 0:
            ...
            save_pickle_file(parsed_data, 'parsed_data')
...
def load_embeddings(embeddings_index, filepath):
    ...
    # notEncoded = io.open(filepath, "r", encoding='utf-8')                                      // edited again.........................................
    # encoded = notEncoded.encode("utf-8")
    with io.open(filepath, 'r', encoding='utf8') as f:
        for line in f:
            values = line.split(" ")
            wordCounter = values[0]
            embedding = np.asarray(values[1:], dtype='float32')
            embeddings_index[wordCounter] = embedding

    length = len(str(embeddings_index))
    print('Word embeddings:', length)
...
def create_conversion_dictionaries(word_frequency, embeddings_index, threshold=10):
    ...
    for token, freq in word_frequency.items():
        if freq > threshold and token not in embeddings_index:
            missing_words += 1
...
def remove_wrong_length_data(coverted_inputs, converted_targets, vocab2int,
    ...
    print('Doing final preprocessing - sorting the texts and keeping only those ' + 'of appropriate length.')
    for length in range(start_inputs_length, max_inputs_length):
        for index, words in enumerate(converted_targets):
            if (min_targets_lengths <= len(converted_targets[index]) <= max_targets_length and
...
data_inputs = load_pickle_file('train_squad_paragraphs')
data_targets = load_pickle_file('train_squad_questions')
...
# ==============================================================================
try:
    parsed_inputs = load_pickle_file('parsed_inputs')
    parsed_targets = load_pickle_file('parsed_targets')
except:
    print('Preprocessing inputs, this may take a while...')
    parsed_inputs = preprocess_data(data_inputs, remove_stopwords=True, replace_entities=True)
    save_pickle_file(parsed_inputs, 'parsed_inputs')
    print('Preprocessing targets, this may take a while...')
    parsed_targets = preprocess_data(data_targets, remove_stopwords=False, replace_entities=True)
    save_pickle_file(parsed_targets, 'parsed_targets')

    assert len(parsed_inputs) == len(parsed_targets)
    print('Loaded up {} parsed inputs/targets pairs 2'.format(len(parsed_inputs)))
...
filepath = 'data/numberbatch-en.txt'
# 'data/' + filename
embeddings_index = {}
...
save_pickle_file(vocab2int, 'vocab2int')
save_pickle_file(int2vocab, 'int2vocab')
...
save_pickle_file(word_embedding_matrix, 'word_embedding_matrix')
...
converted_inputs, word_count, unk_count = convert_data_to_ints(parsed_inputs, vocab2int, word_count, unk_count)
converted_targets, word_count, unk_count = convert_data_to_ints(parsed_targets, vocab2int, word_count, unk_count)
...
save_pickle_file(converted_inputs, 'converted_inputs')
save_pickle_file(converted_targets, 'converted_targets')
...
sorted_inputs, sorted_targets = remove_wrong_length_data(converted_inputs, converted_targets, vocab2int,
                                                         start_inputs_length=min(summary_inputs.counts),
                                                         max_inputs_length=int(
                                                             np.percentile(summary_inputs.counts, 100)),
                                                         max_targets_length=int(
                                                             np.percentile(summary_targets.counts, 100)),
...
save_pickle_file(sorted_inputs, 'sorted_inputs')
save_pickle_file(sorted_targets, 'sorted_targets')
