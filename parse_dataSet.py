...
# def readFiles():
#     numberOfFiles = len(fileList.filename)
#     pdfToTxt = open('pdfToText.txt', 'w+')
#     content = " "
#     for count in range(0, numberOfFiles):
#
#         # print(fileList.getFileName(count))
#         pdfFileObj = open(fileList.getFileName(count), 'rb')
#         # creating a pdf file object
#         pdfReader = PyPDF2.PdfFileReader(pdfFileObj)
#         # creating a pdf reader object
#
#         print("Total number of pages in " + fileList.getFileName(count) + " is : " + str(pdfReader.numPages))
#
#         numberOfPages = pdfReader.numPages
#         # printing number of pages in pdf file
#
#         num = 0
#         while num <= numberOfPages - 1:
#             content += pdfReader.getPage(num).extractText() + " \n"
#             content = u" ".join(content.replace(u"\xa0", u" ").strip().split())
#             num = num + 1
#             # extracting text from pages
#         pdfFileObj.close()
#     pdfToTxt.write(content.encode("ascii", "xmlcharrefreplace"))
#     pdfToTxt.close()
